...
export default {
	...
	data() {
		return {
			...
			content: '<div id="mapDialog_container" ref="mapDialog_container" style="width: 300px;height:200px;"></div>',
			markerData: { test: "测试" },
			pane: null,
			gem: [],
		};
	...
	methods: {
		...
		extendPopup() {
			...
			let arr = [
				{
					...
					data: "//sf1-cdn-tos.huoshanstatic.com/obj/media-fe/xgplayer_doc_video/flv/xgplayer-demo-360p.flv",
			...
			this.gem = [];
			for (let i = 0; i < arr.length; i++) {
				const marker = L.marker(arr[i].point, { icon: this.mapUrlB }).addTo(this.map);
