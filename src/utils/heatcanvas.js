...
HeatCanvas.prototype._createWorker = function () {
  console.log("123123-------", HeatCanvas.getPath());
...
HeatCanvas.prototype._render = function (f_value_color) {
  ...
  if (!f_value_color) {
    ...
    var t = this;
    makeColor = function (value) {
      return t._defaultPalette[Math.round(value * t._paletteSize)];
    };
  } else
    makeColor = function (value) {
      return HeatCanvas.hsla2rgba.apply(null, f_value_color(value));
    };
  ...
  if (
    !(this.value instanceof Float32Array) ||
    this.width != this._valueWidth ||
    this.height != this._valueHeight
  ) {
    // canvas was resized while worker was computing heatmap
    return;
  }
  ...
  if (valLen != this._valueWidth * this._valueHeight) {
    console.error(
      "HeatCanvas: inconsistent data w*h != len",
      this._valueWidth,
      this._valueHeight,
      "!=",
      valLen
  ...
  for (var id = 0; id < valLen; id++) {
    if (this.value[id] > maxValue) maxValue = this.value[id];
  }

  // MDC ImageData:
  // data = [r1, g1, b1, a1, r2, g2, b2, a2 ...]
  for (
    var pos = 0, pixelColorIndex = 0;
    pos < valLen;
    pos++, pixelColorIndex += 4
  ) {
...
HeatCanvas.prototype._ensureDefaultPalette = function () {
  if (
    ...
    this._defaultPalette.length == this._paletteSize + 1
  )
    return this;

  var res = [];
  for (var j = 0; j <= this._paletteSize; j++) {
    var c = HeatCanvas.hsla2rgba.apply(
      null,
      HeatCanvas.defaultValue2Color(j / this._paletteSize)
    );
